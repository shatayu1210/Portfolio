<?php
// echo json_encode(['code'=>200, 'msg'=>'Yayy']);
// Turn off all error reporting
// error_reporting(0);
// $errorMSG="";
include dirname(dirname(__FILE__)).DIRECTORY_SEPARATOR.'includes'.DIRECTORY_SEPARATOR.'dbconn.php';
// echo "Herecds,mskdlm;lsdklsmk;lmdkdsmd;;ksjvkjdsnk";

// echo json_encode(['code'=>404, 'msg'=>'asdsdscdc']);
...
$rblock = mysqli_real_escape_string($con,$_POST['block']);
    ...
    echo $rvillage;
//--------------------
$long123 = mysqli_real_escape_string($con,$_POST['long123']);
$lat123  = mysqli_real_escape_string($con,$_POST['lat123']);
...
if(!empty($_POST["place"])  && !empty($_POST["date1"]) ){
  ...
  if (count($name) == count($material)) {
      for ($i=0; $i < count($name); $i++) {
        ...
        if($query45)
{
  echo json_encode(['code'=>200, 'msg'=>'Query Travel History']);
} 
...
if(!empty($_POST["place2"])  && !empty($_POST["date2"]) ){
   ...
   if (count($name) == count($material)) {
       for ($i=0; $i < count($name); $i++) {
          ...
          if($query45)
{
  echo json_encode(['code'=>200, 'msg'=>'Query Travel Family']);
}
          mysqli_query($con, $query45);
    }
  }
 ...
 if(!empty($_POST["name3"])  && !empty($_POST["age3"]) && !empty($_POST["sex3"]) ){
     ...
     if (count($name) == count($material)) {
         for ($i=0; $i < count($name); $i++) {
            ...
            if($query45)
{
  echo json_encode(['code'=>200, 'msg'=>'Nearby']);
}
            mysqli_query($con, $query45);
      }
    }
