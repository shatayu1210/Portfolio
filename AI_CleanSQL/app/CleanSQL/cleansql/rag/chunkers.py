...
def _format_schema(profile: dict) -> List[Chunk]:
    """Format schema information as RAG chunks."""
    chunks: List[Chunk] = []
    for table in profile.get("tables", []):
        ...
        chunks.append(Chunk(
            text=text,
            library="Structure",
            metadata={"table": table["name"], "db_id": profile["db_id"]}
        ))
    return chunks


def _format_health(profile: dict) -> List[Chunk]:
    """Format health metrics as RAG chunks."""
    chunks: List[Chunk] = []
    for column in profile.get("health", []):
        ...
        chunks.append(Chunk(
            text=text,
            library="HealthRules",
            metadata={"table": column["table"], "column": column["column"], "db_id": profile["db_id"]}
        ))
    return chunks


def _format_values(profile: dict) -> List[Chunk]:
    """Format value dictionaries as RAG chunks."""
    chunks: List[Chunk] = []
    for col_dict in profile.get("dictionaries", []):
        ...
        chunks.append(Chunk(
            text=text,
            library="ValuesUnits",
            metadata={"table": col_dict["table"], "column": col_dict["column"], "db_id": profile["db_id"]}
        ))
    return chunks


def _format_exemplars(profile: dict) -> List[Chunk]:
    """Format exemplar SQL patterns as RAG chunks."""
    ...
    for i, shot in enumerate(exemplars):
        ...
        chunks.append(Chunk(
            text=text,
            library="Exemplars",
            metadata={"db_id": profile["db_id"], "shot_id": str(i)}
        ))
    return chunks


def build_chunks(profile: dict) -> List[Chunk]:
    """Combine all libraries into a single chunk list."""
    return (
        _format_schema(profile) +
        _format_health(profile) +
        _format_values(profile) +
        _format_exemplars(profile)
    )


def build_chunk_payload(profile: dict) -> List[dict]:
    """Build payload dicts for Qdrant ingestion."""
    payloads = []
