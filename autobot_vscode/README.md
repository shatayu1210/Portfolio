...
```bash
cd autobot_dev/autobot_vscode/local_orchestrator
python3 -m venv .venv
source .venv/bin/activate
pip install -r requirements.txt
cp .env.example .env
...
1. Open the folder **`autobot_dev/autobot_vscode`** in VS Code / Cursor.
...
| Build errors for extension | Run `npm install` and `npm run compile` in `autobot_vscode`. |
